...
import { getManagementItem } from "../../../_lib/management";
...
export default function PartnerManagementDetail() {
    const params = useParams<{ id: string }>();
    const router = useRouter();
    const item = getManagementItem(params.id);

    // 초기 상태를 목업 status 로부터 파생
    const initial = useMemo(() => {
        if (!item) return { started: false, ended: false, done: false };
    ...
    const startAt = `${item.dateLabel} ${item.startTime}`;
    const endAt = `${item.dateLabel} ${item.endTime}`;
