 ...
 *
 *  ⏱ **실행 시간 예산 — 31초.** 이 라우트만은 ⑤~⑦ 이 함수 실행 제한 안에서
 *     전부 끝나야 한다. 중간에 잘리면 승인은 났는데 ⑥ 이 안 돌아 "돈은
 *     빠지고 예약은 미확정" 이 되고, ⑦ 의 되돌림도 사고 기록도 남지 않는다.
 *       승인 20초(APPROVE_TIMEOUT_MS) + 복구 10초(RECOVERY_TIMEOUT_MS) + DB ≈ 1초
 */
 ...
 * 실행 상한 60초.
 *
 *  플랫폼 기본은 300초다(Fluid Compute). 그대로 두면 어딘가 멈춘 요청이
 *  5분을 붙잡고, 그동안 사용자는 결제창 앞에서 결과를 못 본다. 예산 31초
 *  위로 여유만 남기고 끊는다 — 여기까지 왔으면 이미 정상이 아니다.
 *
 *  ⚠️ 어댑터 타임아웃보다 **반드시 커야 한다.** 작아지면 함수가 먼저 죽어
 *     망취소·사고기록이 실행되지 않는다.
 */
